...
FHbase = get_map(location=c(-110.45,31.42,-110.3,31.6), zoom = 12, maptype="terrain-background")

FHF12 <- ggmap(FHbase)

X11()
FHF12+ geom_point(data = ER_LatLong[c(23:30), ], aes(x = lon , y= lat), shape = 21, color = "blue")+
  geom_point(data = OCH_lat_lon[c(1:5), ], aes(x = Longitude, y = Latitude), shape = 22, cex = 2, color = "red")+
  geom_text(data = ER_LatLong[c(23:30), ], aes(label = data.Sensor))+
