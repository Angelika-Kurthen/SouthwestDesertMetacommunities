####################################################
# Code to map for Fort Huachuca, AZ data, Fall 2013
####################################################
require(ggplot2)
require(ggmap)

range(FH_Fall_2013$Latitude)
range(FH_Fall_2013$Longitude)
FHbase = get_map(location=c(-110.45,31.42,-110.3,31.55), zoom = 12, maptype="terrain-background")

FHF13 <- ggmap(FHbase)

X11()
FHF13+ geom_point(data = FH_Fall_2013, aes(x = Longitude , y= Latitude, fill = FlowPermanence), shape = 21,cex = 4) +
  scale_fill_gradient2(low = "red", mid = "yellow", high = "blue", midpoint = 0.5, na.value = NA) +
  labs(x="Longitude", y="Latitude", title= "Fort Huachuca, AZ - Fall 2013") + # label the axes
