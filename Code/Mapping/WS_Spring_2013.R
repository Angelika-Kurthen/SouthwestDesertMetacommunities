##################################################
# Code to map for White Sands, NM data, spring 2013
###################################################
require(ggplot2)
require(ggmap)

range(WS_Spring_2013$Latitude)
range(WS_Spring_2013$Longitude)
...
X11()
WSS13 + geom_point(data = WS_Spring_2013, aes(x = Longitude , y= Latitude, fill = FlowPermanence), shape = 21,cex = 4) +
  scale_fill_gradient2(low = "red", mid = "yellow", high = "blue", midpoint = 0.5, na.value = NA) +
  labs(x="Longitude", y="Latitude", title= "White Sands, NM - Spring 2013") + # label the axes
